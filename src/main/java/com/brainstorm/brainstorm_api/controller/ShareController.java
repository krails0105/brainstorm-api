...
public class ShareController {
    ...
    @GetMapping("/{token}")
    public ResponseEntity<ApiResponse<RoomResponse>> joinRoomByToken(@PathVariable String token) {
        UUID userId = (UUID) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        Room room = shareService.joinRoomByShareToken(token, userId);
        return ResponseEntity.ok(ApiResponse.success(RoomResponse.of(room)));
    }
}
